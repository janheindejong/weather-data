SELECT 
    timestamp - timestamp%{interval_seconds} timestamp,
...
HAVING 
    timestamp - timestamp%{interval_seconds} >= {timestamp_after} AND timestamp - timestamp%{interval_seconds} <= {timestamp_before}
ORDER BY 
    timestamp ASC
;